...
public class FreezeCmd implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        if (args.length < 1) { mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#8FA3BF>Usage: <white>/freeze <player>"); return true; }
        ...
        mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» " + state + " <white>" + target.getName() + ".");
        mm.sendRaw(target, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#8FA3BF>You have been " + (frozen ? "<#D4727A>frozen" : "<#7ECFA0>unfrozen") + " <#8FA3BF>by staff.");
        return true;
    }
