...
public class IpMuteCmd implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length < 1) { mm.sendRaw(sender, "<#D4727A>MOD <dark_gray>»</dark_gray> <#8FA3BF>/ipmute <player> [duration] [reason] [-s]"); return true; }
        Player target = Bukkit.getPlayer(args[0]);
        if (target == null) { mm.sendRaw(sender, "<#D4727A>MOD <dark_gray>»</dark_gray> <#D4727A>Player must be online for IP mute."); return true; }
        ...
        if (Main.getGroupLimitManager().exceedsMaxDuration(sender, "IPMUTE", parsed.duration)) { mm.sendRaw(sender, "<#D4727A>MOD <dark_gray>»</dark_gray> <#D4727A>Duration exceeds your group's maximum."); return true; }
        ...
        mm.sendRaw(sender, "<#D4727A>MOD <dark_gray>»</dark_gray> <#7ECFA0>IP Muted <white>" + target.getName() + " <#8FA3BF>" + durationStr);
        return true;
    }
