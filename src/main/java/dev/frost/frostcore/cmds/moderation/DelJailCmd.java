...
public class DelJailCmd implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        if (args.length < 1) { mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#8FA3BF>Usage: <white>/deljail <name>"); return true; }
        String name = args[0].toLowerCase();
        ModerationManager mod = ModerationManager.getInstance();
        if (mod.getJailLocation(name) == null) { mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#D4727A>Jail not found: <white>" + name); return true; }
        mod.deleteJailLocation(name);
        mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#7ECFA0>Jail <white>" + name + " <#7ECFA0>has been deleted.");
        return true;
    }
