...
public class WarnCmd implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length < 1) { mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#8FA3BF>Usage: <white>/warn <player> [reason] [-s] [-t template]"); return true; }
        ...
        if (parsed.template != null) {
            ...
            if (template == null) { mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#D4727A>Template not found: <white>" + parsed.template); return true; }
            mod.punish(PunishmentType.WARN, target.getUniqueId(), target.getName(), null, template.reason(), sender, -1, parsed.silent);
            mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#7ECFA0>Warned <white>" + target.getName() + " <#8FA3BF>using template <white>" + parsed.template);
            return true;
        }
        ...
        mm.sendRaw(sender, "<gradient:#D4727A:#A35560>MODERATION</gradient> <dark_gray>» <#7ECFA0>Warned <white>" + target.getName() + ".");
        return true;
    }
