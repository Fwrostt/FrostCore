...
import dev.frost.frostcore.utils.TpaUIHelper;
...
public class TpaHereCmd implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (target.getPersistentDataContainer().has(TpaToggleCmd.TPA_DISABLED_KEY, org.bukkit.persistence.PersistentDataType.BYTE)) {
            mm.sendRaw(player, "<red>" + target.getName() + " has teleport requests disabled.");
            return true;
        }

        if (inviteManager.hasInviteFrom(target.getUniqueId(), InviteType.TPA_HERE, player.getUniqueId())) {
        ...
        TpaUIHelper.sendTpaRequest(target, player.getName(), "wants you to teleport to them", expiry);
